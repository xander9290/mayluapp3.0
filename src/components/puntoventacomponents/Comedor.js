...
import AbrirComedorModal from "../modals/AbrirComedor";
...
export default function Comedor(props) {
  ...
  const [valueedit, setValueedit] = useState({ torreta: "" });
  ...
  const handleEditar = async (e) => {
    ...
    if (cuenta.id) {
      let existe = false;
      cuentas.map((cuenta) => {
        if (cuenta.torreta === valueedit.torreta) {
          existe = !existe;
        }
      });
      if (existe) {
        alert("ya existe una cuenta con el mismo nombre");
      } else {
        const data = {
          ...cuenta,
          torreta: valueedit.torreta,
        };
        const res = await axios.put(apiURI + "/cuentas/" + cuenta.id, data);
        setValueedit({ torreta: "" });
        commit(
          "ha editado el nombre de la orden " + cuenta.orden,
          operadorSession
        );
        setCuenta(res.data);
        loadcuentas();
